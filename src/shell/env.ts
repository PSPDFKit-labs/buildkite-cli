...
export function getBuildkiteTokenFromEnv(): string {
  ...
  throw new Error(
    `missing buildkite token. set BUILDKITE_TOKEN/BUILDKITE_API_TOKEN/BK_TOKEN or run 'bkci auth setup'. looked for config at ${configPath}. create a token at https://buildkite.com/user/api-access-tokens with scopes: read_builds, read_build_logs, read_artifacts`
  );
}
